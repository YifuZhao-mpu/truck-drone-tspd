# Matched-budget HGA-TAC (TSPDroneHGATAC.jl) side. Same protocol as run_sota_matched.jl:
# launch complete default-settings runs until the nominal deadline; every replicate
# (cost + cumulative time) recorded; hard cutoff applied symmetrically in the analyzer.
# Usage: julia run_sota_hgatac.jl <sota_input.txt> <out.txt> <blk>
...
while i <= length(lines)
    ...
    if blk == want
        ...
        redirect_stdout(devnull) do   # suppress the package's per-run prints
            TSPDroneHGATAC.solve_tspd(rand(10), rand(10); truck_speed=1.0, drone_speed=2.0)
        end                                                     # JIT warmup off the clock
        T = budget(n, kind)
        ...
        while time() - t0 < T
            ...
            el = time() - t0
            best = min(best, r.best_total_cost)
            push!(reps, string(r.best_total_cost, ":", round(el, digits=2)))
        end
        open(out, "w") do io
